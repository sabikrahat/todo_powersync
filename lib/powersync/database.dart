...
final class ListItemWithStats {
  ...
  const ListItemWithStats(this.self, this.completedCount, this.pendingCount);
}
...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration {
    return MigrationStrategy(
      onCreate: (m) async {
        ...
        await createFts5Tables(db: this, tableName: 'lists', columns: ['name']);
        await createFts5Tables(db: this, tableName: 'todos', columns: ['description', 'list_id']);
      },
  ...
  Future<void> addTodoPhoto(String todoId, String photoId) async {
    await (update(todoItems)
      ..where((t) => t.id.equals(todoId))).write(TodoItemsCompanion(photoId: Value(photoId)));
  }
